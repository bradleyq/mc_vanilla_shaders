...
void main() {
    ...
    vec2 RealProjMat = vec2(tan(decodeFloat(texture(DataSampler, start + CTL_ATAN_PMAT00 * inc).xyz)), tan(decodeFloat(texture(DataSampler, start + CTL_ATAN_PMAT11 * inc).xyz)));

    float near = PROJNEAR;
    float far = float(decodeInt(texture(DataSampler, start + CTL_FARCLIP * inc).xyz));
