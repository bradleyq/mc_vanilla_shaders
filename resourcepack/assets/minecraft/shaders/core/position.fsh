...
void main() {
    ...
    // not a control pixel, draw sky like normal
        ...
        skycol.rgb += (1.0 - length(skycol.rgb)) * (vec3(0.01, 0.02, 0.06) + noise);

        fragColor = linear_fog_real(skycol, pow(1.0 - ndusq, 4.0), 0.0, 1.0, FogColor);
    }
    ...
    // draw stars with random colors
        ...
        fragColor = mix(starColor, vec4(starColor.rgb, 0.0), clamp(length(uv - vec2(0.5)) * 2.0, 0.0, 1.0));
    }
    else {
