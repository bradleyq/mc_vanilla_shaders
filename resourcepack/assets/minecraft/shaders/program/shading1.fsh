...
vec3 JoinedBilateralGaussianBlur(vec2 uv1, vec2 duv, float sigX, float sigY, float sigV, float stride, bool x)
{   

    float total = 0.0;
    vec3 ret = vec3(0.0);

    float depth = texture(DiffuseDepthSampler, duv).r;
    ...
    for (float i = -BLF_SAMPLES * stride; i <= BLF_SAMPLES * stride; i+= stride)
    {
        ...
        float sd = texture(DiffuseDepthSampler, duv + vec2(offsetx, offsety)).r;
                    
        float fv = Gaussian( sigV, abs(sd - depth) );
        
        total += fg*fv;
        ret += fg*fv * vec3(texture(DiffuseSampler, uv1 + vec2(offsetx, offsety / 2.0)).r);
    }
...
void main() {
    ...
    outColor = vec4(JoinedBilateralGaussianBlur(normCoord1, texCoord, st_dev, st_dev, sigV, stride, DirX > 0.5), 1.0);
