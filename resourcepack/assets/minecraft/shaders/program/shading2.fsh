...
void main() {
    ...
    if (length(sunDir) > 0.99) {
        ...
        if (!isSky) {
            ...
            outColor.rgb *= texture(ShadingSampler, vec2(texCoord.x, texCoord.y * 0.5 + 0.5)).rgb;

            // apply lighting color. not quite standard diffuse light equation since the blocks are already "pre-lit"
            vec3 shade = min(texture(ShadingSampler, vec2(texCoord.x, texCoord.y * 0.5)).rgb, texture(ShadingSampler, vec2(texCoord.x, texCoord.y * 0.5 + oneTexel.y * 0.5)).rgb);
